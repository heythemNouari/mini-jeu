...
franchissable_env coord (Envi cnt) = if ((contenu M.! coord ) == Monstre x ) then false 
                                                                    else true
...
trouve_id x env = if ((M.findWithDefault (Joueur 50) x) == (Joueur 50)) then Nothing
                                                                      else Just (M.findWithDefault (Joueur 50) x)
